// api/analyze-news.js - VERSION OFFICIELLE WEB SEARCH
export default async function handler(req, res) {
    ...
    try {
        ...
        const tickersMap = { all: ['SPY', 'QQQ'], health: ['XLV', 'JNJ'], tech: ['QQQ', 'AAPL'], crypto: ['BTC-USD', 'ETH-USD'], industrial: ['XLI', 'CAT'], energy: ['XLE', 'XOM'], finance: ['XLF', 'JPM'], defensive: ['XLP', 'PG'] };
        const tickers = tickersMap[sector] || tickersMap.all;

        const prompt = `Analyste financier. Date: ${new Date().toLocaleDateString('fr-FR')}.
Trouve 5 nouvelles financières RÉELLES (<48h) sur "${sectorName}".
UTILISE LA RECHERCHE WEB pour des liens RÉELS.
Réponds UNIQUEMENT en JSON: {"articles": [{"title": "...", "summary": "...", "source": "Bloomberg", "time": "2h", "link": "https://...", "tickers": ["${tickers[0]}", "${tickers[1]}"]}]}`;
        ...
        if (!response.ok) {
            ...
            return res.status(500).json({ error: "GROQ_API_ERROR", details: err });
        }
        ...
        if (!content && data.choices[0]?.message?.tool_calls) {
            content = data.choices[0].message.tool_calls.find(t => t.function?.arguments)?.function?.arguments || "";
        }

        const start = content.indexOf('{');
        const end = content.lastIndexOf('}');
        if (start === -1) return res.status(200).json({ success: true, articles: [], sector: sectorName });
        ...
        return res.status(500).json({ success: false, error: "CRASH", message: error.message });
    }
}
